...
__launch_bounds__(128, 2) __global__ void causal_attention_small_t_tc_partial_bf16_kernel(
    ...
    if constexpr (CacheInput::writes_cache) {
        // Each query column publishes its own row. Other CTAs consume new rows from input,
        // never from a cache write that may still be in flight.
            ...
            if (key >= split_start && key < split_end) {
                if constexpr (CacheInput::writes_cache) {
                    ...
                    if (from_new) {
                        ...
                        store_vec(v_dst, bf16x8_bits_to_f16x8_bits(load_vec<int4>(&input.v[off])));
                    } else {
    ...
    for (int n = 0; n < PVNt; ++n) {
        ...
        if (row0 < row_count) {
            ...
            const std::int64_t dst =
                causal_partial_acc_index<Geometry>(q_head, d0, column + token, split, tokens);
